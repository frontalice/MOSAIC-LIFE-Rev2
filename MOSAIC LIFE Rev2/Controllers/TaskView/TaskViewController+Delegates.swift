...
extension TaskViewController : UITableViewDelegate {
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if !self.taskView.taskListView.isEditing {
...
extension TaskViewController : NSFetchedResultsControllerDelegate {
    ...
    func controllerWillChangeContent(_ controller: NSFetchedResultsController<NSFetchRequestResult>) {
        self.taskView.taskListView.beginUpdates()
    }
    
    func controllerDidChangeContent(_ controller: NSFetchedResultsController<NSFetchRequestResult>) {
        self.taskView.taskListView.endUpdates()
    }
    ...
    func controller(_ controller: NSFetchedResultsController<NSFetchRequestResult>, didChange sectionInfo: NSFetchedResultsSectionInfo, atSectionIndex sectionIndex: Int, for type: NSFetchedResultsChangeType) {
        ...
        switch type {
        case .insert:
            self.taskView.taskListView.insertSections(sections as IndexSet, with: .automatic)
        case .delete:
            self.taskView.taskListView.deleteSections(sections as IndexSet, with: .automatic)
        default:
            return
        }
    }
    ...
    func controller(_ controller: NSFetchedResultsController<NSFetchRequestResult>, didChange anObject: Any, at indexPath: IndexPath?, for type: NSFetchedResultsChangeType, newIndexPath: IndexPath?) {
        switch type {
        case .insert:
            self.taskView.taskListView.insertRows(at: [newIndexPath!], with: .automatic)
        case .update:
            self.taskView.taskListView.reloadRows(at: [indexPath!], with: .none)
        case .delete:
            self.taskView.taskListView.deleteRows(at: [indexPath!], with: .automatic)
        default:
            break
        }
    }
